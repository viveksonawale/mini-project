...
const Leaderboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const [h, entries] = await Promise.all([
          hackathonService.getHackathonById(hackathonId),
          leaderboardService.getLeaderboard(hackathonId),
        ]);
        setHackathon(h);
        setRanked(entries);
      } catch {
                ...
                >
                  <Card className={`text-center border-border/50 ${actualRank === 1 ? "ring-2 ring-accent/50" : ""}`}>
                    <CardContent className="p-5 space-y-2">
                      ...
                      <h3 className="font-heading font-bold text-sm">{sub.teamName}</h3>
                      <p className="text-xs text-muted-foreground">{sub.teamName}</p>
                      <p className="text-2xl font-bold font-heading">{sub.score}<span className="text-xs text-muted-foreground font-normal">/40</span></p>
                    </CardContent>
                  </Card>
                </motion.div>
              );
...
const RankCard = ({ submission: sub, rank, index }: { submission: LeaderboardEntry; rank: number; index: number }) => {
  const criteria = [
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="font-heading font-semibold">{sub.teamName}</h3>
              </div>
              <p className="text-sm text-muted-foreground">{sub.teamName} • {sub.members.length} members</p>
              <div className="flex flex-wrap gap-1 mt-1">
                {sub.members.slice(0, 3).map((m) => <Badge key={m.id} variant="outline" className="text-xs">{m.name}</Badge>)}
              </div>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-3">
                {criteria.map((c) => (
                  <div key={c.key} className="flex items-center gap-2">
                    ...
                    <Progress value={(sub.score / 40) * 100} className="h-1.5 flex-1" />
                    <span className="text-xs font-medium w-4 text-right">-</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="text-right shrink-0">
              <p className="text-3xl font-bold font-heading">{sub.score}</p>
