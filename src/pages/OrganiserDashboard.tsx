...
const OrganiserDashboard = () => {
  const [orgHackathons, setOrgHackathons] = useState<HackathonItem[]>([]);

  useEffect(() => {
    hackathonService.getMyOrganizedHackathons().then(setOrgHackathons).catch(() => setOrgHackathons([]));
  }, []);

  const dashboardStats = useMemo(
    () => [
      { label: "Hackathons", value: orgHackathons.length, change: "", icon: "zap" },
      { label: "Participants", value: orgHackathons.reduce((a, h) => a + (h.participants || 0), 0), change: "", icon: "users" },
      { label: "Teams", value: orgHackathons.reduce((a, h) => a + (h.teams || 0), 0), change: "", icon: "file-text" },
      { label: "Completed", value: orgHackathons.filter((h) => h.status === "COMPLETED").length, change: "", icon: "trophy" },
    ],
    [orgHackathons]
  );
  ...
  const participationTrend = useMemo(
    () =>
      orgHackathons.slice(0, 6).map((h) => ({
        month: h.title.slice(0, 8),
  ...
  const domainDistribution = useMemo(() => {
    ...
    orgHackathons.forEach((h) => (h.themes || []).forEach((t) => map.set(t, (map.get(t) || 0) + 1)));
                ...
                <tbody>
                  {orgHackathons.map((h) => (
                    <tr key={h.id} className="border-b border-border/50 last:border-0">
                      ...
                      <td className="py-3">
                        <Badge variant="secondary" className={`capitalize ${statusColor[h.status] || "bg-muted text-muted-foreground"}`}>{h.status}</Badge>
                      </td>
                      ...
                      <td className="py-3 text-right space-x-1">
                        {(h.status === "ACTIVE" || h.status === "COMPLETED") && (
