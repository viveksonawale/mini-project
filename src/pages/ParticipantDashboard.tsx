...
import { userBadges } from "@/data/badgeMockData";
import { userCertificates } from "@/data/certificateMockData";
import { useAuth } from "@/contexts/AuthContext";
import { hackathonService } from "@/services/hackathonService";
import { teamService, type Invitation } from "@/services/teamService";
...
const ParticipantDashboard = () => {
  ...
  const [teamRequests, setTeamRequests] = useState<Invitation[]>([]);

  useEffect(() => {
    const load = async () => {
      try {
        const [joined, invitations] = await Promise.all([
        ...
        const detailedRegistrations = await Promise.all(
          joined.map(async (h) => {
            ...
            try {
              [teamData, submissionData] = await Promise.all([
                teamService.getMyTeam(h.id),
                submissionService.getMySubmission(h.id).catch(() => null),
              ]);
            } catch (e) {
              console.error(`Failed to fetch details for hackathon ${h.id}`, e);
            }
            ...
            return {
              ...
              skills: h.themes || [],
              githubRepo: submissionData?.githubUrl || "",
              status: submissionData ? "submitted" : "registered",
            };
          })
        );

        setMyRegistrations(detailedRegistrations);
        setTeamRequests(invitations.filter((i) => i.status === "PENDING"));
      } catch {
        setMyRegistrations([]);
        setTeamRequests([]);
      }
  ...
  const participantStats = useMemo(
    () => ({
      hackathonsJoined: myRegistrations.length,
      submissions: myRegistrations.filter((r) => r.status === "submitted").length,
      wins: 0,
      teamRequests: teamRequests.length,
    }),
    [myRegistrations, teamRequests]
  );
                ...
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-sm font-bold text-primary-foreground">
                      {t.invitedBy.name?.charAt(0) || "U"}
                    </div>
                    <div>
                      <p className="font-medium text-sm">{t.invitedBy.name}</p>
                      <div className="flex gap-1 mt-0.5">
                        <Badge variant="outline" className="text-[10px] px-1.5 py-0">{t.teamName}</Badge>
                      </div>
                    </div>
                  </div>
                  <p className="text-sm text-muted-foreground">Invited to join {t.teamName} in {t.hackathonTitle}</p>
                  <div className="flex gap-2">
                    <Button size="sm" className="flex-1 gap-1 bg-accent text-accent-foreground hover:bg-accent/80" onClick={async () => {
                      try {
                        await teamService.acceptInvitation(t.id);
                        setTeamRequests((prev) => prev.filter((x) => x.id !== t.id));
                        toast({ title: "Invitation accepted" });
                      } catch (e) {
                        toast({ title: "Failed to accept", description: e instanceof Error ? e.message : "Please retry", variant: "destructive" });
                      }
                    }}><Check className="h-3 w-3" />Accept</Button>
                    <Button size="sm" variant="outline" className="flex-1 gap-1" onClick={async () => {
                      try {
                        await teamService.rejectInvitation(t.id);
                        setTeamRequests((prev) => prev.filter((x) => x.id !== t.id));
                        toast({ title: "Invitation rejected" });
                      } catch (e) {
                        toast({ title: "Failed to reject", description: e instanceof Error ? e.message : "Please retry", variant: "destructive" });
                      }
